...
- 注 2 ：驻点流（Plane stagnation point flow）提出与最早研究者。
...
卡门认为他在1911~1912年，对这一问题研究的贡献主要是两个方面：
...
物理的直觉告诉卡门，流动稳定的振荡与组合旋涡，即点涡系的稳定性有关。两道对称排列但反方向的旋涡的互相诱导明显不稳定，那么如果两道旋涡交错排列呢？
...
\label{eq6}
$$
事实上，式 $\eqref{eq5}$ 的推导可根据**「单排点列对该列任意点涡诱导合速度为零」**而更加简化，这一结论容易由对称性得到。
...
下面采用 ***Mathematica 12.3*** 软件分别绘制 $b/a = 0.00, \;0.05, \;0.15, \;0.2806, \;0.35, \;0.50, \;1.00, \;1.50$ 六种情形下的流线分布，并设置 $a = 1, \; \Gamma = 1$，如图 [2](#fig2) 所示，源代码见<a href="#附录">附录</a>。可以观察到，该点涡系流线呈现在上下两排点涡间交替穿插的特点。随着 $b/a$ 增加，两排点涡间的流线扭曲程度逐渐趋缓。当 $b/a = 0$ 时，该点涡系退化为单排点涡系。值得注意的是，当 $b/a = 0.2806$ 时，该点涡系正是由冯·卡门提出的**唯一能够保持无粘中性稳定**的情形，即为著名的**卡门涡街 (von Kármán Vortex Street)**。有关点涡系稳定性的推导这里从略，详细请参考相关文献。
...
后来，在卡门基础上，进一步研究发现，圆柱绕流卡门涡街（如图 [3](#fig3) 所示）的形成，**其来流必须满足一定条件**。
